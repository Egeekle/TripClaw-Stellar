...
/**
 * ⚡ PERFORMANCE: Moved static intents outside to avoid re-allocation on every render.
 */
const DEMO_INTENTS = [
...
/**
 * ⚡ Bolt: Pushed simulation state down from Dashboard to LiveFeed.
 * This prevents the entire Dashboard from re-rendering every 3.5 seconds
 * when in demo mode.
 */
export default function LiveFeed({ events = [], isGatewayOnline }) {
  ...
  useEffect(() => {
    // If we have real events or gateway is online, don't simulate
    if (isGatewayOnline && events.length > 0) {
      if (demoEvents.length > 0) setDemoEvents([]);
      return;
    }
  ...
  }, [isGatewayOnline, events.length, demoEvents.length]);
