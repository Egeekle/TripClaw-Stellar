...
/**
 * ⚡ PERFORMANCE: Moved static demo data outside the component to avoid
...
/**
 * ⚡ PERFORMANCE: LiveFeed now manages its own demo interval state.
 * This is "State Colocation" - by pushing high-frequency updates (every 3.5s)
 * down to this leaf component, we prevent the entire Dashboard and its
 * other children (AgentHero, PageHeader, etc.) from re-rendering unnecessarily.
 */
export default function LiveFeed({ events: externalEvents, isGatewayOnline }) {
  const [demoEvents, setDemoEvents] = useState([]);

  useEffect(() => {
    // Only run demo interval if we don't have real gateway events
    if (isGatewayOnline && externalEvents && externalEvents.length > 0) return;
  ...
  const activeEvents = isGatewayOnline && externalEvents && externalEvents.length > 0
    ? externalEvents
    : demoEvents;
