...
const LiveFeed = memo(function LiveFeed({ events: externalEvents, isGatewayOnline }) {
  // Simulated live feed when gateway is offline (demo mode)
  const [demoEvents, setDemoEvents] = useState([]);

  useEffect(() => {
    // Only run demo simulation if gateway is offline and no external events provided
    ...
    <section className="space-y-4 pb-8">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded">
          {isGatewayOnline ? 'Tiempo Real' : 'Modo Demo'}
        </span>
      </div>
