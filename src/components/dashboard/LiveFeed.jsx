...
// PERFORMANCE: DEMO_INTENTS is moved outside the component to avoid recreating the array on every render.
const DEMO_INTENTS = [
...
export default function LiveFeed({ events }) {
  // PERFORMANCE: Localizing demo state here (State Colocation) prevents the entire
  // Dashboard parent from re-rendering every 3.5 seconds during demo mode.
  const [demoEvents, setDemoEvents] = useState([]);

  useEffect(() => {
    // If we have real events from the gateway, we don't need the demo simulation
  ...
  const activeEvents = events && events.length > 0 ? events : demoEvents;
