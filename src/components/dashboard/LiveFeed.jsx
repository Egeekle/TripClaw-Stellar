...
/**
 * PERFORMANCE: Moved static data outside the component to avoid recreation on every render.
 * Static constants should not live inside the functional component body.
 */
const DEMO_INTENTS = [
...
/**
 * PERFORMANCE: State Colocation applied here.
 * The high-frequency demo interval (every 3.5s) is now localized in LiveFeed.
 * This ensures that these updates only re-render this specific component,
 * shielding the Dashboard and its other children (AgentHero, Quick Stats, etc.)
 * from unnecessary render cycles.
 */
export default function LiveFeed({ agentEvents = [], isGatewayOnline }) {
  const [demoEvents, setDemoEvents] = useState([]);

  useEffect(() => {
    // If gateway is online and has real events, we don't need simulated ones
