 ...
 * redundant allocation on every render cycle.
 */
const DEMO_INTENTS = [
...
/**
 * ⚡ Bolt: Pushed state down from Dashboard to LiveFeed.
 * This isolates high-frequency simulation updates (every 3.5s) to this component only,
 * preventing unnecessary re-renders of the entire Dashboard, PageHeader, and Hero.
 */
export default function LiveFeed() {
  const { isGatewayOnline, agentEvents } = useOpenClaw();
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-0.5">
                  <p className="font-bold text-sm text-slate-900 dark:text-white">@{event.agent || event.tool}</p>
                  <p className="text-[10px] font-medium text-slate-400">{event.time}</p>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {event.action || event.content}
                </p>
              </div>
