...
export function OpenClawProvider({ children }) {
  ...
  // PERFORMANCE: Memoizing the context value prevents all consumer components
  // from re-rendering whenever OpenClawProvider re-renders (e.g. on status polling),
  // unless one of the actual state values in the dependency array changes.
  // This is a high-leverage optimization for central state providers.
  const value = useMemo(() => ({
    status,
    wsStatus,
    tools,
