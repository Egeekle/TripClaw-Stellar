...
export function OpenClawProvider({ children }) {
  ...
  /**
   * ⚡ PERFORMANCE: Memoized context value to prevent unnecessary re-renders of all consumer components.
   * Without useMemo, a new object is created on every OpenClawProvider render,
   * causing all consumers to re-render even if the actual state hasn't changed.
   */
  const value = useMemo(() => ({
    status,
    wsStatus,
